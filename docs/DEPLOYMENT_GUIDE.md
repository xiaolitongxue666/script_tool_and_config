...
> **两阶段 Agent 部署（CLI + MCP）** 见 [docs/DEPLOY_TWO_PHASE.md](DEPLOY_TWO_PHASE.md)（Phase 2 详见 agent-config 同文档）。本指南侧重 chezmoi 日常运维与 Windows ↔ Arch 同步。
...
```bash
# 在 Windows Git Bash 中
./scripts/deploy_utils/sync_to_remote.sh
```
...
# 或直接使用 chezmoi
# chezmoi 不读 CHEZMOI_SOURCE_DIR；原文的 `export ...` 写法无效
./scripts/manage_dotfiles.sh edit
```
...
```bash
# 添加到 ~/.zshrc
# chezmoi 不读该变量；改用 ./scripts/manage_dotfiles.sh apply（勿写入 shell 启动文件）
source ~/.zshrc
```
