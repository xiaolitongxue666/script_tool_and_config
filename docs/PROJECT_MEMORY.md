...
5) **Pi**：`run_once_94` + chezmoi `dot_pi/agent/`（默认 v4 flash；`models.json` `modelOverrides` 须 **contextWindow 1M**（`1000000`），勿用 64K 否则会压 pi 框架层上下文）；`/login` 须 **Use an API key** → DeepSeek（订阅列表无 DeepSeek）；`.gitignore` 忽略 `.pi/` 用户态；Pi ≥0.75 需 Node ≥22.19。
