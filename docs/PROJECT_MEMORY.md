...
6) **代理（默认启用）**：唯一入口 `chezmoi_core.sh` → `chezmoi_setup_proxy`；WSL → `http://<resolv nameserver>:7890`；**headless 原生 Linux（VPS）** 扫描 `PROXY_PROBE_PORTS`（含 **17890**）并注入 chezmoi override-data；其余桌面 Linux/macOS/Windows → `127.0.0.1:7890`；禁用 `PROXY=none/false` 或 `NO_PROXY=1`；**brew 操作在 `upgrade_brew_package/upgrade_brew_cask` 中临时 unset 代理**（`common_install.sh`），避免 brew 因全局代理不可用而挂起。
...
25) **部署排错**：`deploy.sh`/`manage_dotfiles.sh apply` 须经 `chezmoi_run_apply`；apply 后 `chezmoi status` 的 M/R 可为正常差异；winget msstore 证书警告非致命；**VPS SSH Host `xiaolitongxue-vps`**：`id_ed25519_github_personal` 优先 + `id_ed25519` 回退；SSH `SetEnv TERM` 需 sshd AcceptEnv，靠 `~/.pi_ssh_helpers.sh` / `.zprofile` 在 SSH 会话设 TERM；VPS 验收见 `diagnose_vps_ssh_pi.sh`。
