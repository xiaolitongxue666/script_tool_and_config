...
| 方式 | **全平台含 WSL**：`npm install -g codewhale`（上游官方推荐） |
...
- `DEEPSEEK_TUI_RELEASE_BASE_URL` — postinstall 二进制镜像（见上游 [INSTALL.md](https://github.com/Hmbown/CodeWhale/blob/main/docs/INSTALL.md)）
...
工作区 overlay：`./.codewhale/config.toml` 优先，`.deepseek/config.toml` 为上游回退。
...
Layer 4 字母序：`90-install-claude-code` → `92-install-codewhale` → `93-install-cursor`。三者独立，可按需使用。

## 迁移与排错（常见问题）

| 现象 | 处理 |
|------|------|
| 仓库已更新但本机无 `codewhale` | 执行 `./scripts/manage_dotfiles.sh apply`，或手动 `npm install -g codewhale`（7890 代理开启） |
| Windows 新终端找不到命令 | 确认 `~/AppData/Roaming/npm` 在 PATH（chezmoi 应用 `_bash_profile_windows.tmpl` 后重开终端） |
| `doctor` 仅识别 `~/.deepseek` | 运行 apply 触发 run_once 迁移，或见 [PROJECT_AGENT_MEMORY.md](PROJECT_AGENT_MEMORY.md) |
| 误用 `codewhale setup --migrate` | v0.8.47 无此参数；由 run_once 脚本复制状态 |

Agent 维护本功能时请阅读 [PROJECT_AGENT_MEMORY.md](PROJECT_AGENT_MEMORY.md)。

## 故障排查

1. 确认本机或 WSL 宿主机 **7890** 代理已启动。
2. 测试 GitHub：`curl -x http://127.0.0.1:7890 -I https://github.com`
3. WSL：`curl -x http://$(awk '/^nameserver / {print $2; exit}' /etc/resolv.conf):7890 -I https://github.com`
4. 检查命令：`command -v codewhale codewhale-tui`、`npm list -g codewhale`
5. 遗留 `deepseek` 命令来自旧 cargo 包，可手动卸载；请改用 `codewhale`。
