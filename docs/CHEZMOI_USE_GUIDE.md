...
```bash
# Linux 特定配置
.chezmoi/run_on_linux/dot_config/i3/config.tmpl

# macOS 特定配置
.chezmoi/run_on_darwin/dot_yabairc.tmpl

# Windows 特定配置
.chezmoi/run_on_windows/_bash_profile_windows.tmpl
```
...
```bash
# 通用安装脚本
.chezmoi/run_once_install-starship.sh.tmpl
...
- `dot_config/i3/config.tmpl` - i3wm 窗口管理器配置
- `run_once_install-i3wm.sh.tmpl` - i3wm 安装脚本
- `run_once_install-dwm.sh.tmpl` - dwm 安装脚本

### macOS

macOS 特定的配置和脚本位于 `run_on_darwin/` 目录：

- `dot_yabairc.tmpl` - Yabai 窗口管理器配置
- `dot_skhdrc.tmpl` - skhd 快捷键配置
- `run_once_install-yabai.sh.tmpl` - Yabai 安装脚本
- `run_once_install-skhd.sh.tmpl` - skhd 安装脚本
...
Cursor 编辑器 `User/settings.json` **不由本仓库 chezmoi 管理**（已迁移至 **agent-config**；`run_once_93-install-cursor` 仅安装 GUI/CLI）：
