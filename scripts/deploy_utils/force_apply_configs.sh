...
# ============================================
CONFIG_MAPPINGS_SH="${PROJECT_ROOT}/scripts/lib/chezmoi/config_mappings.sh"
...
process_file() {
    ...
    if [ -f "$target_file" ]; then
        ...
        if chezmoi apply --force "$target_file" 2>/dev/null; then
...
if [[ -f "$USER_CHEZMOI_CONFIG" ]]; then
    log_info "执行: chezmoi apply -v --force --config ${USER_CHEZMOI_CONFIG}"
    APPLY_OUTPUT=$(chezmoi apply -v --force --config "$USER_CHEZMOI_CONFIG" 2>&1)
else
    log_info "执行: chezmoi apply -v --force --source ${CHEZMOI_SOURCE_DIR}"
    APPLY_OUTPUT=$(chezmoi apply -v --force --source "$CHEZMOI_SOURCE_DIR" 2>&1)
fi
APPLY_EXIT_CODE=$?
...
if [ "$all_exist" = true ]; then
    ...
    log_info "建议运行: ./scripts/deploy_utils/diagnose_deployment.sh 进行诊断"
fi
