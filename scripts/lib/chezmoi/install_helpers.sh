...
get_chezmoi_status_summary() {
    ...
    # 统计不同类型的变更
    # 注意：grep -c 无匹配时已输出 0 且 exit 1，故用 || true 吞退出码；
    # 写成 || echo "0" 会让变量变成 "0\n0"（换行污染日志）
    ...
    local run=$(echo "$status_output" | grep -c "^R" || true)
...
get_chezmoi_diff_summary() {
    ...
    # 统计差异文件数量（同上：grep -c 已输出 0，勿再 echo "0"）
    local file_count=$(echo "$diff_output" | grep -c "^diff --git" || true)
...
extract_software_name_from_script() {
    ...
    # 平台前缀归一：run_once_<platform>-* → run_once_*
    # 平台专属脚本命名规则为 run_once_{linux,macos,windows}-<原语义>，
    # 例如 run_once_macos-install-ghostty.sh.tmpl 应解析出 ghostty（而非 macos-install-ghostty），
    # 否则平台过滤 / [5/6] 报告别名 / 升级策略都会失配。
    case "$basename" in
        run_once_linux-*|run_once_macos-*|run_once_darwin-*|run_once_windows-*)
            basename="run_once_${basename#run_once_*-}"
            ;;
    esac
...
script_applicable_to_platform() {
    ...
    # 平台专属脚本：按文件名平台前缀判定（2026-09 起 run_on_* 目录已取消）
    local _bn
    _bn="$(basename "$script_path")"
    case "$_bn" in
        run_once_linux-*)
            [[ "$plat" != "linux" ]] && return 1
            ;;
        run_once_macos-*|run_once_darwin-*)
...
check_script_software_installed() {
    ...
    case "$software_name" in
        ...
        clangd|install-clangd)
            # 仅检测 clangd 二进制；Cursor 扩展由 scripts/tools/cursor_clangd/ 另装
