...
# 通用日志/错误处理函数库
# 必须先于下列子模块加载：子模块内的兜底日志函数带 `if ! type log_info` 守卫，
# 真库先到位才能让兜底跳过，否则 run_once 脚本会静默用内联 fallback。
# 注意：common.sh 内含 readonly 定义，重复 source 会因只读变量重赋值报错，
# 故以其独有函数 echo_color_message 判断是否已加载。
_COMMON_SH_PATH="${_COMMON_INSTALL_DIR%/*}/common.sh"
if ! declare -F echo_color_message >/dev/null 2>&1 && [[ -f "${_COMMON_SH_PATH}" ]]; then
...
load_run_once_context() {
    ...
    if [[ -z "$project_root" ]]; then
        # 向上查找直到找到 scripts/lib/common.sh
        local probe_dir="$caller_script_dir"
        for _ in 1 2 3 4 5; do
            if [[ -f "${probe_dir}/scripts/lib/common.sh" ]]; then
    ...
    local common_install="${COMMON_INSTALL:-${project_root}/scripts/lib/chezmoi/common_install.sh}"
    if [[ ! -f "$common_install" ]]; then
        common_install="$HOME/.local/share/chezmoi/scripts/lib/chezmoi/common_install.sh"
    fi
