...
chezmoi_get_source_dir() {
    # 本文件位于 scripts/lib/chezmoi/，向上三级到仓库根（P4 迁移后调整）
...
chezmoi_sync_windows_terminal_config() {
    ...
    # 唯一来源：chezmoi 管理的 ~/.config/windows-terminal/settings.json
    # （历史 ~/run_on_windows/... 兜底已随 2026-09 重构移除，该路径不再产生）
    local src="$HOME/.config/windows-terminal/settings.json"
...
# 约定：函数结果走 stdout（供 $(...) 捕获），日志必须走 stderr，否则会污染调用方变量
chezmoi_run_status() {
    echo "[INFO] Checking config status (chezmoi status)..." >&2
    chezmoi_capture_status
}
...
# 约定：函数结果走 stdout（供 $(...) 捕获），日志必须走 stderr，否则会污染调用方变量
chezmoi_run_diff() {
    echo "[INFO] Checking config diff (chezmoi diff)..." >&2
    chezmoi_capture_diff
}
