...
```bash
cd scripts/tools/container_dev_env
...
1. **在 WSL2 中进入项目目录**
   ...
   # 或使用 --windows-path 参数
   ./scripts/tools/container_dev_env/run-wsl.sh --windows-path "C:\path\to\project"
   ```

2. **启动容器**
   ```bash
   # 使用默认代理
   ./scripts/tools/container_dev_env/run-wsl.sh
   
   # 使用指定代理
   ./scripts/tools/container_dev_env/run-wsl.sh --proxy 192.168.1.76:7890
   
   # 执行命令
   ./scripts/tools/container_dev_env/run-wsl.sh --command "nvim"
   ```
...
1. **换行符问题**: 文件包含 CRLF 换行符
   - 解决：运行 `./scripts/tools/standalone_tool_script/ensure_lf_line_endings.sh` 规范化换行符
