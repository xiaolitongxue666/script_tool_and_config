...
if command -v clangd >/dev/null 2>&1; then
    ...
    log_error "clangd not in PATH — run: bash scripts/tools/cursor_clangd/install_clangd.sh"
    errors=$((errors + 1))
...
if [[ -n "$CURSOR_BIN" ]]; then
    ...
    if "$CURSOR_BIN" --list-extensions 2>/dev/null | grep -qx "$EXTENSION_ID"; then
        ...
        log_warn "  run: bash scripts/tools/cursor_clangd/install_cursor_clangd_extension.sh"
